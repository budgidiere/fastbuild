...
Worker::Worker(void *hInstance, const AString &args)
    : m_MainWindow(nullptr), m_ConnectionPool(nullptr),
      m_NetworkStartupHelper(nullptr), m_BaseArgs(args), m_LastWriteTime(0),
...
#endif
{
  ...
  StatusMessage("FBuildWorker %s (%s)", FBUILD_VERSION_STRING,
                FBUILD_VERSION_PLATFORM);
}
...
Worker::~Worker() {
  ...
  if (m_RestartNeeded) {
    Process p;
    p.Spawn(m_BaseExeName.Get(), m_BaseArgs.Get(),
...
int Worker::Work() {
  ...
  if (m_ConnectionPool->Listen(Protocol::PROTOCOL_PORT) == false) {
    ErrorMessage("Failed to listen on port %u.  Check port is not in use.",
                 Protocol::PROTOCOL_PORT);
    return -1;
  }
  ...
  // We just create this folder whether it's needed or not
  {
    ...
    if (!FileIO::EnsurePathExists(tmpPath)) {
      ErrorMessage("Failed to initialize tmp folder.  Error: 0x%x",
                   Env::GetLastErr());
      return -2;
    }
#if defined(__WINDOWS__)
    ...
    tmpPath += "/_lock";
#endif
    if (!m_TargetIncludeFolderLock.Open(tmpPath.Get(),
                                        FileStream::WRITE_ONLY)) {
      ErrorMessage("Failed to lock tmp folder.  Error: 0x%x",
                   Env::GetLastErr());
      return -2;
    }
  }
...
bool Worker::HasEnoughDiskSpace() {
  ...
  VERIFY(FileIO::GetTempDir(tmpPath));
  BOOL result =
      GetDiskFreeSpaceExA(tmpPath.Get(), (PULARGE_INTEGER)&freeBytesAvailable,
  ...
  // The drive doesn't have enough free space or could not be queried. Exclude
  // this machine from worker pool.
