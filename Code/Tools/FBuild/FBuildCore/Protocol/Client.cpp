...
Client::Client(const Array<AString> &workerList)
    : m_WorkerList(workerList), m_ShouldExit(false), m_Exited(false) {
  // allocate space for server states
  m_ServerList.SetSize(workerList.GetSize());

  m_Thread =
      Thread::CreateThread(ThreadFuncStatic, "Client", (64 * KILOBYTE), this);
  ASSERT(m_Thread);
}
...
void Client::LookForWorkers() {
  ...
  for (size_t j = 0; j < numWorkers; j++) {
    ...
    const ConnectionInfo *ci =
        Connect(m_WorkerList[i], Protocol::PROTOCOL_PORT);
    if (ci == nullptr) {
      ss.m_DelayTimer.Start(); // reset connection attempt delay
    } else {
      const uint32_t numJobsAvailable(
          JobQueue::IsValid()
...
void Client::CommunicateJobAvailability() {
  ...
  if (m_StatusUpdateTimer.GetElapsed() <
      CLIENT_STATUS_UPDATE_FREQUENCY_SECONDS) {
    return;
  }
...
// OnReceive
//------------------------------------------------------------------------------
/*virtual*/ void Client::OnReceive(const ConnectionInfo *connection, void *data,
                                   uint32_t size, bool &keepMemory) {
...
//------------------------------------------------------------------------------
void Client::Process(const ConnectionInfo *connection,
                     const Protocol::MsgRequestJob *) {
...
//------------------------------------------------------------------------------
void Client::Process(const ConnectionInfo *connection,
                     const Protocol::MsgJobResult *, const void *payload,
                     size_t payloadSize) {
  ...
  if (result == true) {
    ...
    if (Node::EnsurePathExistsForFile(nodeName) == false) {
      ...
      const uint32_t secondFileSize =
          on->IsUsingPDB() ? *(uint32_t *)((const char *)data +
                                           sizeof(uint32_t) + firstFileSize)
                           : 0;
      ...
      if (fs.Open(nodeName.Get(), FileStream::WRITE_ONLY) == false) {
        ...
        result = false;
      } else if (fs.WriteBuffer((const char *)data + sizeof(uint32_t),
      ...
      } else if (on->IsUsingPDB()) // is there a second file?
      {
        data = (const void *)((const char *)data + sizeof(uint32_t) +
                              firstFileSize);
        ASSERT((firstFileSize + secondFileSize + (sizeof(uint32_t) * 2)) ==
               size);
        ...
        if (fs2.Open(pdbName.Get(), FileStream::WRITE_ONLY) == false) {
          ...
          result = false;
        } else if (fs2.WriteBuffer((const char *)data + sizeof(uint32_t),
    ...
    if (systemError) {
      ...
      FLOG_INFO("Remote System Failure!\n"
                ...
                " - Node		         : %s\n"
                " - Job Error Count   : %u / %u\n",
                workerName.Get(), job->GetNode()->GetName().Get(),
    ...
    Node::DumpOutput(nullptr, failureOutput.Get(), failureOutput.GetLength(),
                     nullptr);
  }
  ...
  JobQueue::Get().FinishedProcessingJob(
      job, result, true, false); // remote job, not a race of a remote job
}
...
// FindManifest
//------------------------------------------------------------------------------
const ToolManifest *Client::FindManifest(const ConnectionInfo *connection,
  ...
  for (Job **it = ss->m_Jobs.Begin(); it != ss->m_Jobs.End(); ++it) {
...
Client::ServerState::ServerState()
    : m_Connection(nullptr), m_CurrentMessage(nullptr), m_NumJobsAvailable(0),
      m_Jobs(16, true), m_Blacklisted(false) {
