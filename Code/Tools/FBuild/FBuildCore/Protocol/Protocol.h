...
enum {
  SERVER_STATUS_FREQUENCY_MS = 1000
}; // frequency of server status updates to client
enum {
  SERVER_STATUS_TIMEOUT = 50000
}; // server is dead if time elapses between updates	//@KS: Increased timeout
   // to fix disconnections on WIFI
...
enum MessageType {
  ...
  MSG_NO_JOB_AVAILABLE =
      4,       // Server <- Client : Respond that no jobs are available
  MSG_JOB = 5, // Server <- Client : Respond with a job to do

  MSG_JOB_RESULT = 6, // Server -> Client : Return completed job

  MSG_REQUEST_MANIFEST = 7, // Server -> Client : Ask client for the manifest of
  ...
  MSG_SERVER_STATUS =
      11, // Server -> Client : Send status / keep connection alive
...
public:
  ...
  void Send(const ConnectionInfo *connection,
            const MemoryStream &payload) const;
  void Send(const ConnectionInfo *connection,
            const ConstMemoryStream &payload) const;
