...
public:
  inline bool operator()(const Job *job1, const Job *job2) const {
    return (job1->GetNode()->GetRecursiveCost() <
            job2->GetNode()->GetRecursiveCost());
  }
};

// JobSubQueue CONSTRUCTOR
//------------------------------------------------------------------------------
JobSubQueue::JobSubQueue() : m_Count(0), m_Jobs(1024, true) {}
...
JobQueue::JobQueue(uint32_t numWorkerThreads)
    : m_NumLocalJobsActive(0), m_DistributableAvailableJobs(1024, true),
      m_DistributableJobsMemoryUsage(0), m_DistributedJobsRemote(1204, true),
      m_DistributedJobsLocal(128, true), m_DistributedJobsCancelled(128, true),
      m_CompletedJobs(1024, true), m_CompletedJobsFailed(1024, true),
      m_CompletedJobs2(1024, true), m_CompletedJobsFailed2(1024, true),
...
// GetJobStats
//------------------------------------------------------------------------------
void JobQueue::GetJobStats(uint32_t &numJobs, uint32_t &numJobsActive,
...
void JobQueue::QueueJob2(Job *job) {
  ...
  AtomicDecU32(
      &m_NumLocalJobsActive); // job converts from active to pending remote
...
Job *JobQueue::OnReturnRemoteJob(uint32_t jobId, bool &cancelled) {
  ...
  if (it) {
    cancelled = true;
    Job *cancelledJob =
        it->m_Job; // pointer value is ok, but object may no longer exist!
...
void JobQueue::FinalizeCompletedJobs() {
  ...
  for (Job **i = m_CompletedJobs2.Begin(); i != end; i++) {
    Job *job = (*i);
    Node *n = job->GetNode();
    if (n->Finalize()) {
  ...
  for (Job **i = m_CompletedJobsFailed2.Begin(); i != end; i++) {
    Job *job = (*i);
  ...
  if (wasARemoteJob) {
    ...
    if (it) {
      ...
      if (localRaceOfRemoteJob) {
        ...
        ASSERT(m_DistributedJobsRemote.Find(job) ==
               nullptr); // should have been removed in OnReturnRemoteJob
      }
    }
  } else {
    ASSERT(m_NumLocalJobsActive > 0);
...
/*static*/ Node::BuildResult JobQueue::DoBuild(Job *job) {
  ...
  if (node->GetType() == Node::OBJECT_NODE ||
      ...
      node->GetType() == Node::DLL_NODE || node->GetType() == Node::CS_NODE) {
    bStartedVSLog = true;
  ...
  if (node->IsAFile() && (node->GetType() != Node::FILE_NODE) &&
      (node->GetType() != Node::COMPILER_NODE)) {
  ...
  Node::BuildResult result;
  {
    ...
    if (node->GetType() == Node::OBJECT_NODE) {
      ...
      profilingTag = on->IsCreatingPCH()
                         ? "PCH"
                         : on->IsUsingPCH() ? "Obj (+PCH)" : profilingTag;
    }
      ...
      if (node->GetControlFlags() & Node::FLAG_NO_DELETE_ON_FAIL) {
        ...
        if (FileIO::FileExists(node->GetName().Get())) {
          ...
          if (FileIO::FileDelete(node->GetName().Get()) == false) {
            ...
            FLOG_ERROR("Post failure deletion failed for '%s'",
                       node->GetName().Get());
          }
        }
      }
