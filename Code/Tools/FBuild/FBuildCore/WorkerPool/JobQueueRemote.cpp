...
JobQueueRemote::JobQueueRemote(uint32_t numWorkerThreads)
    : m_PendingJobs(1024, true), m_CompletedJobs(1024, true),
      m_CompletedJobsFailed(1024, true), m_Workers(numWorkerThreads, false) {
...
// GetWorkerStatus
//------------------------------------------------------------------------------
void JobQueueRemote::GetWorkerStatus(size_t index, AString &hostName,
                                     AString &status, bool &isIdle) const {
...
void JobQueueRemote::GetQueueStatus(AString &status) {
  ...
  status.Format(" | %d wait | %d proc | %d done | %d fail", pendingJobs,
                inFlightJobs, completedJobs, failedJobs);
}
...
// WakeMainThread
//------------------------------------------------------------------------------
void JobQueueRemote::WakeMainThread() { m_MainThreadSemaphore.Signal(); }
...
// WakeWorkers
//------------------------------------------------------------------------------
void JobQueueRemote::WakeWorkers() { m_WorkerThreadSemaphore.Signal(); }
...
/*static*/ Node::BuildResult JobQueueRemote::DoBuild(Job *job,
  ...
  // Delete any left over PDB from a previous run (to be sure we have a clean
  // pdb)
  ...
  *((uint32_t *)mem.Get()) = size;
  ...
  if (includePDB) {
    ...
    *((uint32_t *)(mem.Get() + sizeof(uint32_t) + size)) = size2;
    ...
    // read second file
    if (fs2.Read(mem.Get() + sizeof(uint32_t) + size + sizeof(uint32_t),
                 size2) != size2) {
      FLOG_ERROR("File read error for '%s'", node->GetName().Get());
      return false;
    }
