...
// CONSTRUCTOR
//------------------------------------------------------------------------------
ObjectNode::ObjectNode(const AString &objectName, Node *inputNode,
                       Node *compilerNode, const AString &compilerArgs,
                       const AString &compilerArgsDeoptimized,
                       Node *precompiledHeader, uint32_t flags,
                       const Dependencies &compilerForceUsing,
                       bool deoptimizeWritableFiles,
                       bool deoptimizeWritableFilesWithToken,
                       bool allowDistribution, bool allowCaching,
                       Node *preprocessorNode, const AString &preprocessorArgs,
                       uint32_t preprocessorFlags)
    : FileNode(objectName, Node::FLAG_NONE), m_Includes(0, true),
      m_Flags(flags), m_CompilerArgs(compilerArgs),
      ...
      m_AllowDistribution(allowDistribution), m_AllowCaching(allowCaching),
      m_Remote(false), m_PCHNode(precompiledHeader),
...
// CONSTRUCTOR (Remote)
//------------------------------------------------------------------------------
ObjectNode::ObjectNode(const AString &objectName, NodeProxy *srcFile,
                       const AString &compilerArgs, uint32_t flags)
    : FileNode(objectName, Node::FLAG_NONE), m_Includes(0, true),
      m_Flags(flags), m_CompilerArgs(compilerArgs),
      m_DeoptimizeWritableFiles(false),
      m_DeoptimizeWritableFilesWithToken(false), m_Remote(true),
      m_PCHNode(nullptr), m_PreprocessorNode(nullptr), m_PreprocessorArgs(),
...
/*virtual*/ bool ObjectNode::DoDynamicDependencies(bool forceClean) {
  ...
  if (forceClean) {
    m_DynamicDependencies
        .Clear(); // We will update deps in Finalize after DoBuild
    return true;
  }
...
/*virtual*/ Node::BuildResult ObjectNode::DoBuild(Job *job) {
  ...
  bool useDist = GetFlag(FLAG_CAN_BE_DISTRIBUTED) && m_AllowDistribution &&
                 FBuild::Get().GetOptions().m_AllowDistributed;
  bool usePreProcessor = (useCache || useDist || GetFlag(FLAG_GCC) ||
                          GetFlag(FLAG_SNC) || GetFlag(FLAG_CLANG) ||
                          GetFlag(CODEWARRIOR_WII) || GetFlag(GREENHILLS_WIIU));
...
// DoBuild_Remote
//------------------------------------------------------------------------------
/*virtual*/ Node::BuildResult
ObjectNode::DoBuild2(Job *job, bool racingRemoteJob = false) {
  ...
  return DoBuildWithPreProcessor2(job, useDeoptimization, stealingRemoteJob,
                                  racingRemoteJob);
}
...
//------------------------------------------------------------------------------
/*virtual*/ Node::BuildResult
ObjectNode::DoBuildMSCL_NoCache(Job *job, bool useDeoptimization) {
  ...
  // spawn the process
  CompileHelper ch;
  if (!ch.SpawnCompiler(job, GetName(), GetCompiler()->GetName(),
                        fullArgs)) // use response file for MSVC
  {
...
Node::BuildResult ObjectNode::DoBuildWithPreProcessor(Job *job,
  ...
  if (BuildPreprocessedOutput(fullArgs, job, useDeoptimization) == false) {
    return NODE_RESULT_FAILED; // BuildPreprocessedOutput will have emitted an
                               // error
  }

  // preprocessed ok, try to extract includes
  if (ProcessIncludesWithPreProcessor(job) == false) {
    return NODE_RESULT_FAILED; // ProcessIncludesWithPreProcessor will have
                               // emitted an error
  }
  ...
  if (GetFlag(FLAG_CAN_BE_DISTRIBUTED) && m_AllowDistribution &&
      FBuild::Get().GetOptions().m_AllowDistributed &&
  ...
  Node::BuildResult result = DoBuildWithPreProcessor2(
      job, useDeoptimization, stealingRemoteJob, racingRemoteJob);
...
Node::BuildResult ObjectNode::DoBuildWithPreProcessor2(Job *job,
  ...
  if (job->IsLocal()) {
    ...
    // The CUDA compiler seems to not be able to compile its own preprocessed
    // output
  ...
  const bool showCommands =
      (FBuild::IsValid() && FBuild::Get().GetOptions().m_ShowCommandLines);
  const bool isRemote = (job->IsLocal() == false);
  if (stealingRemoteJob || racingRemoteJob || verbose || showCommands ||
      isRemote) {
    // show that we are locally consuming a remote job
    EmitCompilationMessage(fullArgs, useDeoptimization, stealingRemoteJob,
                           racingRemoteJob, false, isRemote);
  }
...
// DoBuildOther
//------------------------------------------------------------------------------
/*virtual*/ Node::BuildResult ObjectNode::DoBuildOther(Job *job,
                                                       bool useDeoptimization) {
  // Format compiler args string
  Args fullArgs;
  const bool showIncludes(false);
...
bool ObjectNode::ProcessIncludesMSCL(const char *output, uint32_t outputSize) {
  ...
  FLOG_INFO("Process Includes:\n - File: %s\n - Time: %u ms\n - Num : %u",
            m_Name.Get(), uint32_t(t.GetElapsedMS()),
            uint32_t(m_Includes.GetSize()));
...
bool ObjectNode::ProcessIncludesWithPreProcessor(Job *job) {
...

  {
    ...
    if (GetDedicatedPreprocessor() != nullptr) {
      msvcStyle =
          GetPreprocessorFlag(FLAG_MSVC) || GetPreprocessorFlag(FLAG_CUDA_NVCC);
    } else {
...
/*static*/ Node *ObjectNode::Load(IOStream &stream) {
  ...
  NODE_LOAD(AStackString<>,
            objNameOverride); //@KS: Added Object FileName Override
  ...
  size_t numStaticDepsExcludingForceUsing =
      staticDeps.GetSize() - compilerForceUsing.GetSize();
  // compiler + source file + (optional)precompiledHeader +
  // (optional)preprocessor
  ASSERT((numStaticDepsExcludingForceUsing >= 2) &&
         (numStaticDepsExcludingForceUsing <= 4));
#endif
  ...
  NodeGraph &ng = FBuild::Get().GetDependencyGraph();
  Node *on = ng.CreateObjectNode(
      name, staticDepNode, compiler, compilerArgs, compilerArgsDeoptimized,
      m_PCHNode, flags, compilerForceUsing, deoptimizeWritableFiles,
      deoptimizeWritableFilesWithToken, allowDistribution, allowCaching,
      preprocessor, preprocessorArgs, preprocessorFlags);
...
/*static*/ uint32_t ObjectNode::DetermineFlags(const Node *compilerNode,
  ...
  const bool isDistributableCompiler =
      (compilerNode->GetType() == Node::COMPILER_NODE) &&
      (compilerNode->CastTo<CompilerNode>()->CanBeDistributed());

  // Compiler Type
  if (compiler.EndsWithI("\\cl.exe") || compiler.EndsWithI("\\cl") ||
      compiler.EndsWithI("\\icl.exe") || compiler.EndsWithI("\\icl")) {
    flags |= ObjectNode::FLAG_MSVC;
  } else if (compiler.EndsWithI("clang++.exe") ||
             compiler.EndsWithI("clang++") || compiler.EndsWithI("clang.exe") ||
             compiler.EndsWithI("clang") ||
             compiler.EndsWithI("clang-cl.exe") ||
             compiler.EndsWithI("clang-cl")) {
    flags |= ObjectNode::FLAG_CLANG;
  } else if (compiler.EndsWithI("gcc.exe") || compiler.EndsWithI("gcc") ||
             compiler.EndsWithI("g++.exe") || compiler.EndsWithI("g++")) {
  ...
  } else if (compiler.EndsWithI("\\nvcc.exe") || compiler.EndsWithI("\\nvcc")) {
    flags |= ObjectNode::FLAG_CUDA_NVCC;
  } else if (compiler.EndsWith("rcc.exe") || compiler.EndsWith("rcc")) {
  ...
  if (flags & ObjectNode::FLAG_MSVC) {
    ...
    // 1) clr code cannot be distributed due to a compiler bug where the
    // preprocessed using statements are truncated 2) code consuming the windows
    // runtime cannot be cached due to preprocessing weirdness 3) pch files are
    // machine specific 4) user only wants preprocessor step executed
    if (!usingWinRT && !usingCLR && !usingPreprocessorOnly &&
  ...
  if (flags &
      (ObjectNode::FLAG_CLANG | ObjectNode::FLAG_GCC | ObjectNode::FLAG_SNC |
       ObjectNode::CODEWARRIOR_WII | ObjectNode::GREENHILLS_WIIU)) {
...
/*virtual*/ void ObjectNode::SaveRemote(IOStream &stream) const {
  ...
  // TODO:B would be nice to make ShouldUseDeoptimization cache the result for
  // this build instead of opening the file again.
...
// DumpOutput
//------------------------------------------------------------------------------
/*static*/ void ObjectNode::DumpOutput(Job *job, const char *data,
                                       uint32_t dataSize, const AString &name,
...
const AString &ObjectNode::GetCacheName(Job *job) const {
  ...
  // TODO:B Exclude preprocessor control defines (the preprocessed input has
  ...
  FLOG_INFO("Cache hash: %u ms - %u kb '%s'\n", uint32_t(t.GetElapsedMS()),
            uint32_t(job->GetDataSize() / KILOBYTE), cacheName.Get());
  return job->GetCacheName();
}
  ...
  if (cache) {
    ...
    if (cache->Retrieve(cacheFileName, cacheData, cacheDataSize)) {
      ...
      if (!objFile.Open(m_Name.Get(), FileStream::WRITE_ONLY)) {
        ...
        FLOG_ERROR("Failed to open local file during cache retrieval '%s'",
                   m_Name.Get());
        return false;
      }
      ...
      if (objFile.Write(data, dataSize) != dataSize) {
        cache->FreeMemory(cacheData, cacheDataSize);
        FLOG_ERROR("Failed to write to local file during cache retrieval '%s'",
                   m_Name.Get());
        return false;
      }
      ...
      if (FALSE == SystemTimeToFileTime(&st, &ft)) {
        FLOG_ERROR("Failed to convert file time after cache hit '%s' (%u)",
                   m_Name.Get(), Env::GetLastErr());
        return false;
      }
      ...
      if (timeSetOK == false) {
        FLOG_ERROR("Failed to set timestamp on file after cache hit '%s' (%u)",
                   m_Name.Get(), Env::GetLastErr());
        return false;
      }
      ...
      // granularity for timekeeping, so we need to update with the actual time
      // written
      m_Stamp = FileIO::GetFileLastWriteTime(m_Name);
...
//------------------------------------------------------------------------------
void ObjectNode::EmitCompilationMessage(
    const Args &fullArgs, bool useDeoptimization, bool stealingRemoteJob,
    bool racingRemoteJob, bool useDedicatedPreprocessor, bool isRemote) const {
  ...
  if (FLog::ShowInfo() ||
      (FBuild::IsValid() && FBuild::Get().GetOptions().m_ShowCommandLines) ||
      isRemote) {
    output += useDedicatedPreprocessor
                  ? GetDedicatedPreprocessor()->GetName().Get()
                  : GetCompiler() ? GetCompiler()->GetName().Get() : "";
    output += ' ';
...
// StripTokenWithArg
//------------------------------------------------------------------------------
/*static*/ bool ObjectNode::StripTokenWithArg(const char *tokenToCheckFor,
                                              const AString &token,
                                              size_t &index) {
...
// StripToken
//------------------------------------------------------------------------------
/*static*/ bool ObjectNode::StripToken(const char *tokenToCheckFor,
                                       const AString &token,
...
// BuildArgs
//------------------------------------------------------------------------------
bool ObjectNode::BuildArgs(const Job *job, Args &fullArgs, Pass pass,
                           bool useDeoptimization, bool showIncludes,
                           const AString &overrideSrcFile) const {
  Array<AString> tokens(1024, true);

  const bool useDedicatedPreprocessor =
      ((pass == PASS_PREPROCESSOR_ONLY) && GetDedicatedPreprocessor());
  ...
  const bool isMSVC = (useDedicatedPreprocessor)
                          ? GetPreprocessorFlag(FLAG_MSVC)
                          : GetFlag(FLAG_MSVC);
  const bool isClang = (useDedicatedPreprocessor)
                           ? GetPreprocessorFlag(FLAG_CLANG)
                           : GetFlag(FLAG_CLANG);
  const bool isGCC = (useDedicatedPreprocessor) ? GetPreprocessorFlag(FLAG_GCC)
                                                : GetFlag(FLAG_GCC);
  const bool isSNC = (useDedicatedPreprocessor) ? GetPreprocessorFlag(FLAG_SNC)
                                                : GetFlag(FLAG_SNC);
  const bool isCWWii = (useDedicatedPreprocessor)
                           ? GetPreprocessorFlag(CODEWARRIOR_WII)
                           : GetFlag(CODEWARRIOR_WII);
  const bool isGHWiiU = (useDedicatedPreprocessor)
                            ? GetPreprocessorFlag(GREENHILLS_WIIU)
                            : GetFlag(GREENHILLS_WIIU);
  const bool isCUDA = (useDedicatedPreprocessor)
                          ? GetPreprocessorFlag(FLAG_CUDA_NVCC)
                          : GetFlag(FLAG_CUDA_NVCC);
  const bool isQtRCC = (useDedicatedPreprocessor)
                           ? GetPreprocessorFlag(FLAG_QT_RCC)
                           : GetFlag(FLAG_QT_RCC);
  ...
  for (size_t i = 0; i < numTokens; ++i) {
    ...
    if (isMSVC) {
      if (pass == PASS_COMPILE_PREPROCESSED) {
        // Can't use the precompiled header when compiling the preprocessed
        // output as this would prevent cacheing.
    ...
    if (isMSVC) {
      ...
      if (StripToken("/MP", token,
                     true)) // true = strip '/MP' and starts with '/MP'
      {
        continue;
      }
    ...
    if (pass == PASS_COMPILE_PREPROCESSED) {
      if (isClang) {
        // Clang requires -I options be stripped when compiling preprocessed
        // code (it raises an error if we don't remove these)
      ...
      if (isMSVC) {
        ...
        // (unlike Clang, MSVC is ok with leaving the /I when compiling
        // preprocessed code)
    ...
    if (isMSVC) {
      ...
      if (found) {
        ...
        if (m_ObjNameOverride.IsEmpty()) {
          fullArgs += m_Name;
          fullArgs += GetObjExtension(); // convert 'PrecompiledHeader.pch' to
                                         // 'PrecompiledHeader.pch.obj'
        } else {
    ...
    // It's a common user error to terminate things (like include paths) with a
    // quote this way, messing up the rest of the args and causing bizarre
    // failures. Since " is not a valid character in a path, just strip the
    // escape char
    if (isMSVC) {
      // Is this invalid?
  ...
  if (showIncludes) {
    fullArgs +=
        " /showIncludes"; // we'll extract dependency information from this
  }
  ...
  const AString &compiler =
      job->IsLocal() ? GetCompiler()->GetName() : remoteCompiler;
...
// ExpandTokenList
//------------------------------------------------------------------------------
void ObjectNode::ExpandTokenList(const Dependencies &nodes, Args &fullArgs,
                                 const AString &pre,
                                 const AString &post) const {
...
//------------------------------------------------------------------------------
bool ObjectNode::BuildPreprocessedOutput(const Args &fullArgs, Job *job,
                                         bool useDeoptimization) const {
  const bool useDedicatedPreprocessor = (GetDedicatedPreprocessor() != nullptr);
  EmitCompilationMessage(fullArgs, useDeoptimization, false, false,
                         useDedicatedPreprocessor);
  ...
  if (!ch.SpawnCompiler(job, GetName(),
                        useDedicatedPreprocessor
                            ? GetDedicatedPreprocessor()->GetName()
                            : GetCompiler()->GetName(),
                        fullArgs)) {
  ...
  // take a copy of the output because ReadAllData uses huge buffers to avoid
  // re-sizing
...
//------------------------------------------------------------------------------
void ObjectNode::TransferPreprocessedData(const char *data, size_t dataSize,
                                          Job *job) const {
...
#if defined(__WINDOWS__)
  // VS 2012 sometimes generates corrupted code when preprocessing an already
  // preprocessed file when it encounters enum definitions. Example:
  //		enum dateorder
  //		{
  ...
  if (doVS2012Fixup) {
    ...
    // Now allocate the new buffer with enough space to add a space after each
    // enum found
    newBufferSize = outputBufferSize + nbrEnumsFound;
    bufferCopy =
        (char *)ALLOC(newBufferSize + 1); // null terminator for include parser
...
bool ObjectNode::WriteTmpFile(Job *job, AString &tmpFileName) const {
  ...
  if (WorkerThread::CreateTempFile(tmpFileName, tmpFile) == false) {
    job->Error("Failed to create temp file '%s' to build '%s' (error %u)",
               tmpFileName.Get(), GetName().Get(), Env::GetLastErr);
    job->OnSystemError();
    return NODE_RESULT_FAILED;
  }
  if (tmpFile.Write(dataToWrite, dataToWriteSize) != dataToWriteSize) {
    job->Error("Failed to write to temp file '%s' to build '%s' (error %u)",
               tmpFileName.Get(), GetName().Get(), Env::GetLastErr);
    job->OnSystemError();
    return NODE_RESULT_FAILED;
  }
...
bool ObjectNode::BuildFinalOutput(Job *job, const Args &fullArgs) const {
  ...
  // spawn the process
  CompileHelper ch;
  if (!ch.SpawnCompiler(job, GetName(), compiler, fullArgs,
...
ObjectNode::CompileHelper::CompileHelper(bool handleOutput)
    : m_HandleOutput(handleOutput), m_OutSize(0), m_ErrSize(0), m_Result(0) {}
...
//------------------------------------------------------------------------------
bool ObjectNode::CompileHelper::SpawnCompiler(Job *job, const AString &name,
                                              const AString &compiler,
                                              const Args &fullArgs,
  ...
  // spawn the process
  if (false == m_Process.Spawn(compiler.Get(), fullArgs.GetFinalArgs().Get(),
                               workingDir, environmentString)) {
...
//------------------------------------------------------------------------------
/*static*/ void ObjectNode::HandleSystemFailures(Job *job, int result,
                                                 const char *stdOut,
                                                 const char *stdErr) {
  ...
  if ((result == 0x40010004) || // DBG_TERMINATE_PROCESS
      (result == 0xC0000142)) // STATUS_DLL_INIT_FAILED - Occurs if remote PC is
                              // stuck on force reboot dialog during shutdown
  {
  ...
  {
    job->Error("Remote failure: STATUS_INVALID_IMAGE_FORMAT (0xC000007B) - "
  ...
  if (objectNode->IsMSVC()) {
    ...
    if (stdOut) {
      if (strstr(stdOut, "C1082") || strstr(stdOut, "C1085") ||
          strstr(stdOut, "C1088")) {
        job->OnSystemError();
        return;
      }
...
bool ObjectNode::ShouldUseDeoptimization() const {
  ...
  if (fs.Open(GetSourceFile()->GetName().Get(), FileStream::READ_ONLY)) {
    const size_t bytesToRead =
        Math::Min<size_t>(1024, (size_t)fs.GetFileSize());
    char buffer[1025];
...
bool ObjectNode::ShouldUseCache() const {
  bool useCache = GetFlag(FLAG_CAN_BE_CACHED) && m_AllowCaching &&
                  (FBuild::Get().GetOptions().m_UseCacheRead ||
                   FBuild::Get().GetOptions().m_UseCacheWrite);
  if (GetFlag(FLAG_ISOLATED_FROM_UNITY)) {
    // disable caching for locally modified files (since they will rarely if
    // ever get a hit)
    useCache = false;
  }
  return useCache;
...
#if defined(__WINDOWS__)
  // Generally only windows applications support response files (to overcome
  // Windows command line limits)
  return (GetFlag(FLAG_MSVC) || GetFlag(FLAG_GCC) || GetFlag(FLAG_SNC) ||
          GetFlag(FLAG_CLANG) || GetFlag(CODEWARRIOR_WII) ||
          GetFlag(GREENHILLS_WIIU));
#else
  return false;
#endif
