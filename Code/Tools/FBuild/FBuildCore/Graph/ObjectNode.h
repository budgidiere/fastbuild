...
public:
  explicit ObjectNode(const AString &objectName, Node *inputNode,
  ...
  explicit ObjectNode(const AString &objectName, NodeProxy *srcFile,
  ...
  static uint32_t DetermineFlags(const Node *compilerNode, const AString &args,
                                 bool creatingPCH, bool usingPCH);
  ...
  void SetObjNameOverride(const AString &objNameOverride) {
    m_ObjNameOverride = objNameOverride;
  }
  //@KS: <<<
...
private:
  ...
  BuildResult DoBuildWithPreProcessor(Job *job, bool useDeoptimization,
                                      bool useCache);
  BuildResult DoBuildWithPreProcessor2(Job *job, bool useDeoptimization,
  ...
  static void DumpOutput(Job *job, const char *data, uint32_t dataSize,
                         const AString &name, bool treatAsWarnings = false);

  void EmitCompilationMessage(const Args &fullArgs, bool useDeoptimization,
                              bool stealingRemoteJob = false,
                              bool racingRemoteJob = false,
                              bool useDedicatedPreprocessor = false,
                              bool isRemote = false) const;

  enum Pass { PASS_PREPROCESSOR_ONLY, PASS_COMPILE_PREPROCESSED, PASS_COMPILE };
  static bool StripTokenWithArg(const char *tokenToCheckFor,
                                const AString &token, size_t &index);
  static bool StripToken(const char *tokenToCheckFor, const AString &token,
                         bool allowStartsWith = false);
  bool BuildArgs(const Job *job, Args &fullArgs, Pass pass,
                 bool useDeoptimization, bool useShowIncludes,
                 const AString &overrideSrcFile = AString::GetEmpty()) const;

  void ExpandTokenList(const Dependencies &nodes, Args &fullArgs,
                       const AString &pre, const AString &post) const;
  bool BuildPreprocessedOutput(const Args &fullArgs, Job *job,
                               bool useDeoptimization) const;
  void TransferPreprocessedData(const char *data, size_t dataSize,
                                Job *job) const;
  ...
  inline bool GetPreprocessorFlag(uint32_t flag) const {
    return ((m_PreprocessorFlags & flag) != 0);
  }

  static void HandleSystemFailures(Job *job, int result, const char *stdOut,
                                   const char *stdErr);
  ...
  public:
    ...
    // start compilation
    bool SpawnCompiler(Job *job, const AString &name, const AString &compiler,
                       const Args &fullArgs, const char *workingDir = nullptr);
