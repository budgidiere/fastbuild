...
ConnectionInfo::ConnectionInfo(TCPConnectionPool *ownerPool)
    : m_Socket(INVALID_SOCKET), m_RemoteAddress(0), m_RemotePort(0),
      m_ThreadQuitNotification(false), m_TCPConnectionPool(ownerPool),
      m_UserData(nullptr)
#ifdef DEBUG
      ,
      m_InUse(false)
...
TCPConnectionPool::TCPConnectionPool()
    : m_ListenConnection(nullptr), m_Connections(8, true),
...
void TCPConnectionPool::ShutdownAllConnections() {
  ...
  while (m_ListenConnection || !m_Connections.IsEmpty()) {
...
// GetAddressAsString
//------------------------------------------------------------------------------
/*static*/ void TCPConnectionPool::GetAddressAsString(uint32_t addr,
                                                      AString &address) {
...
bool TCPConnectionPool::Listen(uint16_t port) {
  ...
  int ret = setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (const char *)&yes,
                       sizeof(yes));
  if (ret != 0) {
  ...
  // everything is ok - we are now listening, managing connections on the other
  // thread
  return true;
}
...
// Connect
//------------------------------------------------------------------------------
const ConnectionInfo *TCPConnectionPool::Connect(const AString &host,
                                                 uint16_t port,
                                                 uint32_t timeout) {
  ASSERT(!host.IsEmpty());
...
// Connect
//------------------------------------------------------------------------------
const ConnectionInfo *TCPConnectionPool::Connect(uint32_t hostIP, uint16_t port,
                                                 uint32_t timeout) {
  PROFILE_FUNCTION
...
#endif
  int ret = setsockopt(sockfd, SOL_SOCKET, SO_RCVBUF, (const char *)&bufferSize,
                       sizeof(bufferSize));
  if (ret != 0) {
  ...
  ret = setsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, (const char *)&bufferSize,
                   sizeof(bufferSize));
  if (ret != 0) {
...
#error Unknown platform
  ...
  for (;;) {
    ...
    FD_ZERO(&write);
    FD_ZERO(&err);
    PRAGMA_DISABLE_PUSH_MSVC(6319) // warning C6319: Use of the comma-operator
                                   // in a tested expression...
    FD_SET(sockfd, &write);
    FD_SET(sockfd, &err);
...
// Send
//------------------------------------------------------------------------------
bool TCPConnectionPool::Send(const ConnectionInfo *connection, const void *data,
  ...
  while (bytesToSend > 0) {
    int sent =
        (int)send(connection->m_Socket,
                  ((const char *)&sizeData) + 4 - bytesToSend, bytesToSend, 0);
    if (sent <= 0) {
      if (WouldBlock()) {
        if (connection->m_ThreadQuitNotification || m_ShuttingDown) {
          sendOK = false;
      ...
      TCPDEBUG("send error A.  Send: %i (Error: %i) (%x)\n", sent,
               GetLastError(), connection->m_Socket);
      Disconnect(connection);
      sendOK = false;
      break;
    }
  ...
  if (sendOK) {
    ...
    while (bytesRemaining > 0) {
      int sent = (int)send(connection->m_Socket, dataAsChar,
...
// FreeBuffer
//------------------------------------------------------------------------------
/*virtual*/ void TCPConnectionPool::FreeBuffer(void *data) { FREE(data); }
...
bool TCPConnectionPool::HandleRead(ConnectionInfo *ci) {
  ...
  while (bytesToRead > 0) {
    int numBytes = (int)recv(ci->m_Socket, ((char *)&size) + 4 - bytesToRead,
                             bytesToRead, 0);
    if (numBytes <= 0) {
      if (WouldBlock()) {
...
// Select
//------------------------------------------------------------------------------
int TCPConnectionPool::Select(
    TCPSocket socket,
    void *
        a_ReadSocketSet, // TODO: Using void * to avoid including header is ugly
    void *a_WriteSocketSet, void *a_ExceptionSocketSet,
    timeval *a_TimeOut) const {
  PROFILE_SECTION("Select")
  return select((int)socket, // NOTE: ignored by Windows
                (fd_set *)a_ReadSocketSet, (fd_set *)a_WriteSocketSet,
                (fd_set *)a_ExceptionSocketSet, a_TimeOut);
}
...
// CreateThread
//------------------------------------------------------------------------------
void TCPConnectionPool::CreateListenThread(TCPSocket socket, uint32_t host,
                                           uint16_t port) {
  MutexHolder mh(m_ConnectionsMutex);
  ...
  Thread::ThreadHandle h = Thread::CreateThread(
      &ListenThreadWrapperFunction, "TCPListen", (32 * KILOBYTE),
...
void TCPConnectionPool::ListenThreadFunction(ConnectionInfo *ci) {
  ...
  while (ci->m_ThreadQuitNotification == false) {
    ...
    TCPSocket newSocket = Accept(
        ci->m_Socket, (struct sockaddr *)&remoteAddrInfo, &remoteAddrInfoSize);
    ...
    TCPDEBUG("Connection accepted from %s : %i (%x)\n", addr.Get(),
             ntohs(remoteAddrInfo.sin_port), newSocket);
#endif
...
#error Unknown platform
    ...
    uint32_t bufferSize = (10 * 1024 * 1024);
#endif
    int ret = setsockopt(newSocket, SOL_SOCKET, SO_RCVBUF,
                         (const char *)&bufferSize, sizeof(bufferSize));
    if (ret != 0) {
      TCPDEBUG("setsockopt SO_RCVBUF failed: %i\n", GetLastError());
      break;
    }
    ret = setsockopt(newSocket, SOL_SOCKET, SO_SNDBUF,
                     (const char *)&bufferSize, sizeof(bufferSize));
    if (ret != 0) {
      TCPDEBUG("setsockopt SO_SNDBUF failed: %i\n", GetLastError());
      break;
    ...
    CreateConnectionThread(newSocket, remoteAddrInfo.sin_addr.s_addr,
...
//------------------------------------------------------------------------------
ConnectionInfo *TCPConnectionPool::CreateConnectionThread(TCPSocket socket,
                                                          uint32_t host,
                                                          uint16_t port) {
  MutexHolder mh(m_ConnectionsMutex);
  ...
  getnameinfo((struct sockaddr *)&saGNI, sizeof(struct sockaddr),
              ci->m_RemoteName, NI_MAXHOST, NULL, 0, NI_NUMERICSERV);
  ...
  // Spawn thread to handle socket
  Thread::ThreadHandle h = Thread::CreateThread(
      &ConnectionThreadWrapperFunction, "TCPConnection", (32 * KILOBYTE),
...
//------------------------------------------------------------------------------
/*static*/ uint32_t
TCPConnectionPool::ConnectionThreadWrapperFunction(void *data) {
  ConnectionInfo *ci = (ConnectionInfo *)data;
...
bool TCPConnectionPool::DisableNagle(TCPSocket sockfd) {
  ...
  const int ret = setsockopt(sockfd, IPPROTO_TCP, TCP_NODELAY,
