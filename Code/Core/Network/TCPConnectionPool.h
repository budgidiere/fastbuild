...
public:
  ...
  TCPConnectionPool &GetTCPConnectionPool() const {
    return *m_TCPConnectionPool;
  }
...
#ifdef DEBUG
  mutable bool
      m_InUse; // sanity check we aren't sending from multiple threads unsafely
#endif
};
...
public:
  ...
  // derived classes must call this from their destructor if they rely on
  // virtual callbacks
  ...
  void StopListening();
  const ConnectionInfo *Connect(const AString &host, uint16_t port,
                                uint32_t timeout = 2000);
  const ConnectionInfo *Connect(uint32_t hostIP, uint16_t port,
                                uint32_t timeout = 2000);
  ...
  bool Send(const ConnectionInfo *connection, const void *data, size_t size,
            uint32_t timeoutMS = 2000000); //@KS: Increased Timeout threshold
...
protected:
  ...
  virtual void OnReceive(const ConnectionInfo *, void * /*data*/,
...
private:
  ...
  int Select(TCPSocket maxSocketPlusOne,
             void *readSocketSet, // TODO: Using void * to avoid including
                                  // header is ugly
             void *writeSocketSet, void *exceptionSocketSet,
             struct timeval *timeOut) const;
  TCPSocket Accept(TCPSocket socket, struct sockaddr *address,
  ...
  ConnectionInfo *CreateConnectionThread(TCPSocket socket, uint32_t host,
                                         uint16_t port);
