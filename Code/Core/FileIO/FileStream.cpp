...
// CONSTRUCTOR
//------------------------------------------------------------------------------
FileStream::FileStream() : m_Handle((void *)INVALID_HANDLE_VALUE) {}
...
bool FileStream::Open(const char *fileName, uint32_t fileMode) {
  ...
  // for sharing violations, we'll retry a few times as per
...
#error Unknown platform
  ...
  FSDEBUG(DEBUGSPAM("Open file error: %d - '%s' on %x\n", GetLastError(),
                    fileName, Thread::GetCurrentThreadId());)
...
// IsOpen
//------------------------------------------------------------------------------
bool FileStream::IsOpen() const { return (m_Handle != INVALID_HANDLE_VALUE); }

// Read
//------------------------------------------------------------------------------
/*virtual*/ uint64_t FileStream::ReadBuffer(void *buffer,
  ...
  do {
    ...
    uint32_t tryToReadNow =
        (remaining > MEGABYTE) ? MEGABYTE : (uint32_t)remaining;
    uint32_t bytesReadNow = 0;
    if (FALSE ==
        ReadFile(
            (HANDLE)m_Handle, // _In_         HANDLE hFile,
            (char *)buffer +
                (size_t)totalBytesRead, // _Out_        LPVOID lpBuffer,
...
// Write
//------------------------------------------------------------------------------
/*virtual*/ uint64_t FileStream::WriteBuffer(const void *buffer,
  ...
  do {
    ...
    if (FALSE ==
        WriteFile(
            (HANDLE)m_Handle, // _In_         HANDLE hFile,
            (char *)buffer +
                (size_t)totalBytesWritten, // _In_         LPCVOID lpBuffer,
            tryToWriteNow, // _In_         DWORD nNumberOfBytesToWrite,
            (LPDWORD)&bytesWrittenNow, // _Out_opt_    LPDWORD
...
/*virtual*/ bool FileStream::Seek(uint64_t pos) const {
  ...
  newPos.QuadPart = pos;
  if (FALSE ==
      SetFilePointerEx((HANDLE)m_Handle, newPos, nullptr, FILE_BEGIN)) {
    return false;
  }
  return true;
#elif defined(__APPLE__) || defined(__LINUX__)
...
bool FileStream::SetLastWriteTime(uint64_t lastWriteTime) {
  ...
  ftWrite.dwHighDateTime =
      (uint32_t)((lastWriteTime & 0xFFFFFFFF00000000) >> 32);
  if (!SetFileTime((HANDLE)m_Handle, nullptr, nullptr,
                   &ftWrite)) // create, access, write
  {
    return false;
  }
  return true;
